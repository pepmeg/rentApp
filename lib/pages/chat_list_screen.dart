...
class ChatListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final chats = List<Chat>.from(
      context.watch<ChatProvider>().getChatsForUser(user.id),
    )..sort((a, b) {
            ...
            Expanded(
              child: chats.isEmpty
                ...
                child: Column(
                  ...
                  children: [
                    Icon(Icons.chat_bubble_outline, size: 64, color: AppColors.oliveGray.withOpacity(0.3)),
                    const SizedBox(height: 16),
                    Text('Нет сообщений', style: TextStyle(fontSize: 18, color: AppColors.oliveGray.withOpacity(0.5))),
                ...
                itemBuilder: (context, index) {
                  ...
                  final timeText = lastMessage != null ? DateFormat('HH:mm').format(lastMessage.timestamp) : '';
                  ...
                  Widget buildLeading(UserModel? companion) {
                    Widget leadingWidget = CircleAvatar(
                      radius: 26,
                      backgroundColor: AppColors.oliveGray.withOpacity(0.1),
                      child: const Icon(Icons.chat, color: AppColors.oliveGray, size: 24),
                    );
                    ...
                    if (chat.productImage != null && chat.productImage!.isNotEmpty) {
                      ...
                      if (path.startsWith('assets/')) {
                        leadingWidget = ClipRRect(
                          ...
                          child: Image.asset(path, width: 52, height: 52, fit: BoxFit.cover),
                        );
                      } else {
                        leadingWidget = ClipRRect(
                          borderRadius: BorderRadius.circular(12),
                    ...
                    else if (companion?.avatarPath != null && companion!.avatarPath!.isNotEmpty) {
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      name,
                                      style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.oliveGray),
                                      maxLines: 1,
                                    ...
                                    Text(
                                      lastMessage != null ? lastMessage.text : 'Нет сообщений',
                                      maxLines: 1,
                                      overflow: TextOverflow.ellipsis,
                                      style: TextStyle(
                                        fontSize: 14,
