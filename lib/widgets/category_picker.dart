...
class _CategoryPickerState extends State<CategoryPicker> {
  ...
  Future<void> _restorePath() async {
    ...
    setState(() => _selectedNodes = nodes);
    widget.onPathChanged(path);
  }

  void _selectCategory(CategoryNode node, int level) {
    setState(() {
      _selectedNodes = _selectedNodes.sublist(0, level);
      _selectedNodes.add(node);
      widget.onPathChanged(_selectedNodes.map((n) => n.id).toList());
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i <= _selectedNodes.length; i++) {
      ...
      final String keySuffix = _selectedNodes.take(i).map((n) => n.id).join('_');
      levels.add(
        Padding(
          padding: const EdgeInsets.only(bottom: 10),
          child: AppDropdownMenu(
            key: ValueKey('cat_${i}_${keySuffix}_${children.length}'),
